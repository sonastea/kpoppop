...
const ReportUserModal = (props: { user: ReportUserInfoData }) => {
  ...
  const { reporting, isReporting } = useReportUserStore();
  const {
    reset,
    register,
    handleSubmit,
  ...
  return (
    <Dialog open={reporting} onClose={isReporting} className="relative z-100">
      <div className="fixed inset-0 bg-black/30" aria-hidden="true" />

      <div className="fixed inset-0 flex items-center justify-center">
        <Dialog.Panel className="bg-gray-200 p-6 w-full md:w-3/4 rounded-md">
          <button onClick={isReporting} type="button" className="fixed right-6 md:right-[15%]">
          ...
          {reported ? (
            <>
            <p className="text-sm font-bold text-center border-b border-once-300 pb-2 mb-4">{responseMsg}</p>
            ...
            <form className="grid gap-2" onSubmit={handleSubmit(handleReportUser)}>
              ...
              <div className="flex justify-center">
                <button
                  className="w-full border bg-white border-once rounded-md p-1 mr-1"
                  type="button"
                  onClick={isReporting}
                >
                  Cancel
